...
export default {
  ...
  computed: {
    ...mapState({
      passwordResetEmailSent: (state) => state.accounts.passwordResetEmailSent
    }),
     isSubmitDisabled() {
  ...
  methods: {
    ...mapActions({
      sendPasswordResetEmail: (actions) => actions.accounts.sendPasswordResetEmail,
      clearPasswordResetFailure: (actions) => actions.accounts.clearPasswordResetFailure
    }),
