...
describe('SignUpForm', () => {
  ...
  it('calls actions.accounts.registerUser when submitted, and display success message ', async () => {
    const wrapper = mount(SignUpForm, { store, router });
    wrapper.instance().$Message.success = jest.fn();
    const actions = getActions(wrapper);
    actions.accounts.registerUser = jest.fn(async () => true);
    const firstNameInput = getFirstNameInput(wrapper);
    const lastNameInput = getLastNameInput(wrapper);
    const emailInput = getEmailInput(wrapper);
    const submitBtn = getSubmitBtn(wrapper);
    ...
    await nextTick();
    expect(actions.accounts.registerUser).toHaveBeenCalledWith({
      firstName: 'first',
      lastName: 'last',
      email: 'mail@mail.com'
    });
  ...
  it('calls actions.accounts.clearRegisterFailure() when destroyed', () => {
    const wrapper = mount(SignUpForm, { store, router });
    const actions = getActions(wrapper);
    actions.accounts.clearRegisterFailure = jest.fn();
    wrapper.destroy();
    expect(actions.accounts.clearRegisterFailure).toHaveBeenCalledTimes(1);
  });
});
